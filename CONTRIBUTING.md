...
6. If the skill is a RECIPE (it plans MCP calls), its Step 2 MUST carry the four tool-surface presence artifacts (the per-account tool surface varies; see sw-foundation-core § tool-surface presence): (a) a presence pre-filter ordering line placed before the smoke probe, (b) a pinned absence outcome for EACH REQUIRED tool phrased in the recipe's own tier vocabulary, (c) a smoke-retarget line naming the documented secondary-probe tool, and (d) a standalone-sufficient inline restatement of the call-time unknown-tool rule (trigger substrings, zero retries, the "not exposed on this connector" caveat with both causes), placed in Step 2 OUTSIDE any Cowork-prefixed heading so the build-time strip never removes it. Claude.ai ships recipes without foundations, so the restatement is load-bearing, not decorative.
7. If the recipe outputs richer-than-text data (3+ entities, multi-period, dashboards), optionally add a `## Cowork persistent artifact` section (Tier 3, via `mcp__cowork__create_artifact`) or a `## Cowork chat-side panel` section (Tier 2, via a `.jsx` file written through the Write tool). Reference `docs/cowork-architecture-notes.md` for the contract and CSP rules. Presence-filter the artifact's `mcp_tools` list and pin its minimum-viable rule per sw-foundation-render-cowork.
8. Run `python3 build.py --validate` locally; it fails on description violations, grounding-ledger mismatches, and frontmatter `depends_on:` regressions.
