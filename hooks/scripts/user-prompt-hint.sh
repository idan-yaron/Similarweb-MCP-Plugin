...
# Never block on stdin: read with a short timeout so the script cannot stall to the 30s hook timeout if the harness holds stdin open without writing the prompt.
prompt=""
