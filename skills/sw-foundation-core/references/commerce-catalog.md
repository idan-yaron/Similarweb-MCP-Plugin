...
**Read the category's SCOPE before comparing anything inside it.** The describe returns, per category, a `domains` list and a `description` stating inclusion and exclusion rules. Both are load-bearing and both are free. An empty `domains` list means UNSCOPED (all covered retailers), not unknown; a populated one means the category was built against exactly those retailers, live-observed at 7 US retailers on one category and 9 UK retailers on another. Descriptions carry real exclusions ("Excludes tags: Espresso Machine Accessories, Espresso Machine Replacement Parts"). A brand comparison inside a 7-retailer US category is NOT comparable to one inside an unscoped category, to one scoped to a different market, or to open-web share, and the `market_code` applied is a third axis on top (43 market codes exist). State the basis with any ranking, and where two sides do not share one, say the comparison is not decidable on this panel rather than reporting a winner. Per sw-foundation-render § expert-heuristics comparability.

**`views / number_of_skus` is a denominator trap.** Both fields are available, so a per-listing efficiency figure is easy to compute and easy to get backwards: listing counts include third-party and resale inventory, which inflates the denominator for whichever brand carries more of it. Never rank brands on per-listing efficiency without stating what the listing count contains, and never present it as a like-for-like productivity measure.
