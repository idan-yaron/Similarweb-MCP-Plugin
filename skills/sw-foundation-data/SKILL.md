...
**Two data points count only when they can DISTINGUISH the candidate models.** The 2-plus-points rule above is necessary and not sufficient, and the gap is not hypothetical: `get-websites-serp-players` shipped as "flat 1 credit per call" measured at limits 5 and 25, and `get-keywords-performance` shipped as "flat 1 credit per call" measured at 1 and 3 rows. Both readings had two-plus points and both were wrong, because every point sat inside the SAME pricing block. Re-probed across a boundary, serp-players charges 2 at 100 rows and keywords-performance charges 4 at 52 rows with seven metrics. Before recording a model, ask which observation would have come out differently had the rival model been true, and go measure THAT: two limits on the same side of a step, or two windows on a window-independent tool, are one point wearing a disguise. Grounded in `web-family-cost-shapes-2026-08`.

- **The two-axis shape generalizes: `credits = returned rows + count of NON-NULL metric cells`.** `rows x (metrics + 1)` is the special case where every requested metric came back populated, which is why `get-websites-geography-agg` and the criq performance tool both fit it. `get-pages-leading-folders-agg` shows the general form, because its `change` field IS null on some rows: 15 rows with 2 nulls charged 43 where 15 clean rows charge 45. Six points across three domains fit exactly, and the 2-null point was predicted before the call was issued. Two consequences. A null-heavy response costs LESS than a clean one at the same row count, so a cost figure quoted from one domain can overstate another. And `rows x (metrics + 1)` remains a safe PLANNING upper bound even though it is not the charge. Grounded in `pages-leading-folders-shape` and `subdomains-agg-shape`.

- **Row-priced by RETURNED rows** (the distinction matters when the server has fewer rows than you asked for): `get-website-content-subdomains-agg` charges exactly 2 credits per RETURNED row, window-independent and device-independent. Nine points from 1 to 40 rows scale linearly, and a limit-25 call at `offset: 300` returned 0 rows and charged 0, which is the observation that separates returned-row from requested-row pricing and is the cheapest way to test it on any tool. Grounded in `subdomains-agg-shape`.

- **Priced in BLOCKS of returned cells** (a third shape, distinct from per-row and per-metric): `get-keywords-performance` fits `ceil(rows x requested_metrics / 100)`. The same 52 rows charged 1 credit at one metric and 4 at the default seven, so the lever is the PRODUCT, and trimming `metrics` is the cheapest saving available. A blocked tool looks free until the block boundary, which is exactly what makes it easy to mis-record.
