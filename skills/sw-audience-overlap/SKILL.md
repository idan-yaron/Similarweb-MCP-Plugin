...
| 2 | `get-websites-audience-overlap-agg` | Single batched call, `domains` comma-joined (2-5 total). Returns 2^N - 1 subset rows at ~2 data credits per row (3 domains = 7 rows = ~14 credits; 5 domains = 31 rows = ~62 credits). Warn the user before a 4-5 domain request given the row-count blow-up. |
| 3 | `get-websites-demographics-agg` | Single call for target, `country` is ISO-2. Pass an explicit `start_date`/`end_date` window rather than the server default, which resolves to a multi-year span the render does not use |
| 4 | `get-websites-geography-agg` | Single call for target. ALWAYS pass BOTH `limit: 10` AND `metrics: ["rank", "share", "visits"]` (the render uses the top 10 countries and exactly those three fields: `rank` feeds the not-ranked sentinel and the handoff row, so DO NOT drop it). Priced on rows AND metrics at once, window-independent: `rows x (metrics + 1)` credits, so limit 10 with 3 metrics = 40, and the SAME call with `metrics` omitted costs 70 because the server applies all six (grounded in `geography-agg-cost-shape`, six points across two sessions). Passing `limit` alone is the common mistake and it costs 2.3x. Widen either parameter only if the user explicitly asks for full country coverage or extra engagement fields. |
| 5 | `get-websites-deduplicated-audience` | **Looped per domain** (one call per target + each --against); live tool takes a SINGLE `domain`. BUDGET GATE: about 259 data credits per domain AT the server-default multi-year window with the full metric set (`deduplicated-audience-shape`), the dominant cost of this recipe. Pass an explicit `start_date`/`end_date` window; the render reads the latest row plus the device-mix shares, so the default span is bought and discarded. Narrowing the `metrics` list would cut this further but the exact rendered subset is not yet grounded, so pin the window first. Run by default only when the analysis set is 2 domains; for 3+ domains, skip by default and offer in ONE line ("Deduplicated reach for N domains adds ~259xN data credits; want it?"), running it only on user consent. When skipped, render the section per the budget-skip edge case. |
| 5b | `get-websites-audience-interests-agg` | **Looped per domain** (one call per target + each --against); ALWAYS pass `limit: 15`. About 60 data credits per call AT limit 15 per `audience-interests-shape`; the cost scales with `limit` (about 500 at the server default of 100, per `payload-measurements`), so the bound is the lever and omitting it multiplies both cost and payload by roughly 8. Feeds the Persona overlap (Jaccard) section. Skipped if not accessible; omit section + note in Caveats. |
...
- `## Deduplicated audience`. Per-domain latest-row headline (since looped). For each domain: latest-month `total_deduplicated_audience` and the three device-mix shares (desktop-only, mobile-only, cross-device). GLOSS: `total_deduplicated_audience` (cross-device unique people) is a DIFFERENT methodology from the audience-overlap `union_unique_users` (panel uniques) and the two do not match for the same domain-month (e.g. nike.com 2026-05: ~54.0M dedup vs ~60.9M overlap-union); never present them as the same figure, and when both appear in one render, label which methodology each uses. If section skipped: "Deduplicated audience not accessible on this plan."
