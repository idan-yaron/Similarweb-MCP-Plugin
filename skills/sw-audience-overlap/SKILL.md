...
| 4 | `get-websites-geography-agg` | Single call for target. ALWAYS pass `limit: 10` (the render uses top 10 only). Row-priced at 3 data credits per returned row, window-independent (limit 10 = ~30 credits; grounded in `audience-geography-shape`), so `limit` IS the cost lever (the old ~699 figure was the default limit-100 call). Omit the limit ONLY if the user explicitly asks for full country coverage. |
