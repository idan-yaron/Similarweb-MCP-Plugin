---
name: sw-router
description: Free-form intent classifier for Similarweb shaped prompts that did not name a specific recipe. Auto triggers on the Similarweb keyword surface (web traffic and rank, channels, audience overlap, market size, AEO, similar sites, PPC, keywords, brand sales, category performance, or any Similarweb MCP tool name). When exactly one recipe clearly matches, yield and apply that recipe directly instead of routing. Use the router when intent is ambiguous across recipes, the ask chains two recipes, or no recipe fits and a direct one to three tool MCP sequence answers it. Trivial single domain single metric lookups exit fast via one MCP call with no foundation loads. Asks one crisp clarifier when several recipes fit.
...
- Anything about a PERSON, a contact, an email, enrichment, or outreach (see the hard exclusion below).
- Buying-signal questions ("what changed at X", "what technology does X use", "is X growing").
- Search-demand questions ("how many people search for X", "demand for Y").
- Industry-level questions that name a category rather than a domain.
- Amazon or retailer commerce questions (marketplace sales, ASINs, category leaderboards).

**HARD EXCLUSION: people, contacts, and outreach NEVER take this fast path.** Any prompt about a PERSON at a company, a contact list, an email address, enrichment, or reaching out ("who is the CMO of X", "find me a contact at X", "get me emails for these domains", "enrich this list", "draft outreach to X") proceeds to Step 1 REGARDLESS of how single-entity it looks. This is not a routing preference, it is a safety boundary: Step 0 exits without loading any foundation, so the contact-PII handling rule and the never-auto-invoke rule for `post-emails-outreach` and `post-contacts-bulk` would not be in context at the moment the call is planned. `post-contact-search-contacts` also requires BOTH an explicit `limit` (the server defaults it to 0 and then rejects 0) and an `output_fields` list, neither of which the trivial-lookup defaults supply. A person is not a metric; there is no such thing as a trivial contact lookup here.

**New-surface asks also proceed to Step 1**, because none of them is answerable by one unbounded call with trivial-lookup defaults: buying signals ("what changed at X", "is X hiring", "what tech does X use"), search-demand questions ("how many people search for X"), industry-level demographics, and any Amazon or retailer commerce question ("X's sales on Amazon", "top brands in category Y"). The commerce families need an uncharged discovery call before their analytics call because their ids cannot be guessed (see sw-foundation-core § commerce), and the demand keyword tool has no `limit` at all.
...
The plugin ships 7 user-invocable recipes. The sw-config and sw-setup operator skills are NOT recipes; never route to them from a free-form question. When the platform's own skill matching already surfaced exactly one recipe for the prompt, yield and apply that recipe directly; the router earns its load only for ambiguity, chained plans, and no-fit fallbacks.
...
| `/sw-market-size` | Category sizing on Amazon shopper data (and optional Web companion): demand, leaders, share of category. Prompt uses "market size", "how big is the X market", "who owns the X category", "demand for X", "leaders in X", "top selling products in X", "best sellers", "market for Y on amazon". | "how big is the cybersecurity market in the US", "market size of vegan dog food on amazon", "who owns the streaming category", "top selling products on amazon in home fitness", "demand for noise-canceling headphones" |
| `/sw-aeo-audit` | AI engine visibility (proxy via SEO + SERP + landing pages, optional direct signal upgrade) plus traditional SEO posture. Prompt uses "AEO", "audit aeo", "SEO audit of X", "is X showing up in ChatGPT / Perplexity / Gemini", "do AI assistants recommend X", "are LLMs citing X", "X's SEO posture", "share of voice in AI answers". | "is amazon showing up in ChatGPT answers for laptop reviews", "audit aeo for nytimes.com", "SEO audit of nike.com", "do AI assistants recommend asos.com for fashion", "are LLMs citing webmd.com for health questions" |
...
| `/sw-keyword-opportunity` | Keyword gap analysis for a target, versus a named competitor or auto-discovering the closest organic rival when none is named. Prompt uses "keyword gap", "keyword or SEO opportunities for X", "what keywords does X rank for that Y doesn't", "where can I outrank", "SEO gap analysis". | "what keywords is adidas winning that nike isn't", "find keyword gaps and SEO opportunities for nike.com", "keyword gap between hubspot and salesforce", "where can semrush outrank ahrefs" |
...
**Bound every Branch C call, not just the call count.** The 1-3 call cap bounds how many requests fire; it does nothing about how large each one is, and Branch C is the only general-purpose planner in the plugin, so it reaches tools no recipe has pre-bounded. Apply sw-foundation-core § payload-budget before issuing anything: never plan a never-inline tool, pass an explicit bound to every READ-ONLY tool that accepts one (including tools nobody has measured, where bound-required is the default), and remember the budget binds the TURN, so three bounded calls still have to fit together. Bounding is about shaping a call already decided on; it never makes a side-effectful tool callable. Three specific traps reachable from a free-form prompt: never pass `include_shared: true` to `get-custom-industries-describe` (60 B becomes 5.0 MB); pin `get-sales-signals-news` to a window of at most 7 days (8 KB becomes 1.7 MB); and treat `get-demand-search-trends-keywords-aggregated` as topic-bounded, because it has NO `limit` parameter and returned about 77 KB for zero credits on one common topic, so the only bound is a narrow topic plus a short window. A free charge is not evidence of a small response.
...
**Commerce questions need a discovery call first.** A marketplace or retailer question (Amazon sales, category leaderboards, ASINs, cross-retailer views) cannot be planned straight to an analytics tool: the shopper family needs a numeric category id or brand from `get-categories-search` or `get-brands-search`, and the criq family needs an account-scoped category UUID from `get-retail-cross-analysis-describe`. Both discovery tools are uncharged, so the pattern is one free discovery call plus one bounded analytics call, which fits the 1-3 cap. Do NOT pass a Similarweb domain where a shopper tool asks for `domain`: that parameter names the Amazon marketplace, not the analysed brand. Per sw-foundation-core § commerce.

**Buying-signal questions are one flat-priced call each.** The sales-signals tools charge a flat 10 credits per call regardless of window or row count and accept no `limit`, so the plan decides WHICH signals to fetch, not how much to ask for; two or three signal tools is a complete answer, not a partial one. Pass no dates to `get-sales-signals-intent` (its only valid window is its latest update day; anything else is a 400). An empty result arrives as a POPULATED row carrying a `message` field, so do not report "no data" by testing `len(data)`, and render "no signals in this period" as the real answer it is. Per sw-foundation-core § buying signals.
...
- The contact tools (`post-contact-*`) are the CHEAP path in credits; the company and website lead-enrichment tools are the metered, expensive calls. This is the one Branch C case where more than 3 calls is acceptable, BECAUSE the extra calls are cheap contact lookups, NOT because broadening is encouraged: use them to settle found-or-not-found, then stop.
- **Cheap in credits is not cheap in bytes, and `post-contact-search-contacts` needs TWO bounds.** It returns about 1.9 KB at `limit: 1` and about 69 KB at `limit: 100`, so a wide search is a large payload even though the credit cost is low. ALWAYS pass an explicit `limit`: the parameter is NOT in the schema's required list, but the server defaults it to 0 and then rejects 0 with `400 VALIDATION_ERROR: limit must be greater than 0`, so omitting it always fails (grounded in `contact-search-limit-default`). ALSO pass `output_fields` restricted to the columns the answer actually renders. Unlike `limit`, this one has NO server-side forcing function: omitting it succeeds silently and returns every field the server holds on every matched person. Verified live: passing `output_fields: ["job_title","seniority","department"]` returns those three populated and nulls the rest, including `first_name`, `last_name`, `contact_id`, `linked_in_url`, `city`, and `state` (grounded in `contact-search-limit-default`). Note it NULLS values rather than omitting keys, so the response still lists every field name. If a call ever rejects `output_fields`, do NOT retry without it: report that field restriction is unavailable and ask the user before pulling unrestricted contact records.
- **Contact PII is never persisted anywhere outside the current answer.** Not the capability map, not a handoff, not a file, not a rendered artifact, not a scratch path. Render only the fields the user asked for and drop the rest.
- **`post-emails-outreach` and `post-contacts-bulk` are never planned into a Branch C sequence**, per the never-auto-invoke Hard rule in sw-foundation-core. A prompt that asks to find someone AND contact them is two steps: Branch C may do the finding, then stops and asks.
- **That 400 is a validation error, not a denial.** Per sw-foundation-core § capability-gating it sets nothing in `tools_inaccessible`; recording it as a denial would poison the map and wrongly skip the tool on later runs.
