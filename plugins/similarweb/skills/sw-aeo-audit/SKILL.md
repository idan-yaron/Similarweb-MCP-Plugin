...
- NEVER abort the audit if Step 5 (`--campaign-id` direct signal) returns 4xx. Surface inline, OMIT the Direct AI-engine signal section, add a Caveat, ship the proxy audit.
...
If `--campaign-id` was supplied but `get-gen-ai-campaign-analysis-prompts` is not accessible, skip Step 5 and note in Caveats: "AEO direct-signal tool not accessible on this plan; continuing with proxy audit."
...
Per-keyword fanout: Step 3 fans out to N keywords (default 5-10, capped at 10). Calls are independent; parallelize. Default audit total cost = ~12-13 data credits.
...
Steps 2 and 4 are independent given the resolved windows; parallelize with Step 3. Step 3 fans out per keyword and is itself independent across keywords; parallelize within the loop. Step 5 (if `--campaign-id`) runs in parallel with all proxy calls.

Client-side derivations after responses arrive:

1. **From Step 2 (SEO context):** aggregate per-month rows. Compute:
...
2. **From Step 3 (per keyword):** for each keyword response, locate the row where `domain` matches `$DOMAIN` (root-domain match, ignoring subdomain). Record:
...
3. **From Step 4 (AEO-worthy pages):** for each URL row, compute:
   ```
...
- `## NEXT MOVES` (EXACTLY 2 backtick-quoted natural-language questions, each
  ...
  - **Audience overlap with top AI-cited domains** (only when `--campaign-id` AND Step 5 returned a direct signal):
...
- `direct_ai_signal` is `null` unless `--campaign-id` was supplied AND Step 5 returned 200. Shape when populated:
...
- `_meta.audit_type` is `"proxy"` by default; `"proxy+direct"` when `--campaign-id` was supplied AND Step 5 returned 200. NEVER `"direct"` alone (the proxy IS the audit).
- `_meta.window` is `"3-month rolling"` by default; `"narrower-than-3-months"` for new domains with insufficient history (+ Caveat).
- `_meta.end_date` is the clamped `end_date` from Step 1's `meta.last_updated`.
...
- **Target has no Similarweb coverage** (Step 1 rank returns null): print "Similarweb has no coverage for `<domain>`. Aborting AEO audit." Exit.
