...
**Rung 1: direct measurement, account-dependent.** Attempt `get-ai-traffic-landing-pages-agg` (target, country, latest, `limit: 25`: it is 1 credit per row, so the bound is the cost lever as well as the payload lever, and the server default of 100 would cost about 100 credits). **Then run the composition check before rendering anything**, per sw-foundation-render § expert-heuristics sample composition: `traffic_share` on this tool is normalized against the domain's TOTAL AI traffic and NOT against the returned rows, so `sum(returned shares)` is this sample's coverage and costs nothing to compute. Widen ONCE to `limit: 100` when coverage is below 0.50 or one URL exceeds 0.25 of the total, then stop; a second widening is not worth its credits. Per `ai-landing-pages-composition`, then `get-ai-traffic-overview` (uncharged, but PIN THE WINDOW: its server default reaches back about three years and its rows are months times active LLM sources, so pass the same single effective month the rest of the audit uses). On the reference connector both returned HTTP 403 `FORBIDDEN_ERROR` (uncharged) through 2026-08-06 and 200 from 2026-08-07; that is one account moving, not a rule about accounts. A 403 here means the tool is PRESENT on the server and the AI-research claim is not enabled for this account: render it with exactly that framing per sw-foundation-render § error-rendering Pattern 3, never as "no AI data exists". On a 200, this rung IS the answer: label the section DIRECT measurement and skip rungs 2 and 3 unless the user also asked for channel context. NEVER describe any part of the audit as direct AI measurement unless one of these two tools returned data.

**Rung 1 render rules (all three are mandatory, all three cost nothing).**
1. **State coverage.** Render "these rows are X% of the domain's AI-referred traffic this month" from the summed shares. NEVER present the returned rows as the whole. A low-coverage sample can be perfectly clean and still describe a fifth of the picture.
2. **Split non-marketing surfaces out of the content ranking.** Identity (`/auth/`, `/oauth`, `signin`, `iforgot`, `idmsa`, `account.`, `appstoreconnect`, a `/login` path), support, developer and careers URLs are real traffic and a real finding, but they are NOT answer-engine content performance. Report them on their own line and keep the content table content-only. A brand whose AI traffic is mostly sign-in redirects is being used as an identity provider by assistants, which is worth saying plainly and is not an AEO win.
3. **Lead with composition when one surface dominates.** When a single URL exceeds a quarter of the domain's AI traffic, that surface IS the headline; a ranked content list underneath it would imply a distribution that does not exist.

This is a DETECTOR over the returned rows, never an assumption: one grounded domain returned 8 identity rows in its top 10 while a control domain at the identical shape returned none.
...
Per-keyword fanout: Call 3 fans out to N keywords (default 5-10, capped at 10). Calls are independent; parallelize. Default audit total cost = ~12-13 data credits for the shared SEO and SERP calls, plus the AI rung. On an ENTITLED account rung 1 costs about 25 credits (landing-pages at `limit: 25`, 1 per row; the overview is uncharged), so the entitled default is about 37-38 credits and it is the complete path. The composition check can widen it ONCE to about 100 credits on a domain whose sample is thin or whose head is dominated by one surface; that spend is conditional on a signal, never automatic. On a GATED account rung 2 costs 10 credits for one month, so the gated default is about 22-23 credits as well, but buys a share figure rather than named sources. Both figures are now measured directly: rung 2 at 10 credits per month, and rung 1 at 1 credit per requested row, observed at 10 credits for a 10-row call and 50 for a 50-row call on two separate domains (`ai-landing-pages-composition`). Opt-in rungs are excluded from that default: rung 3 adds 100 credits at `limit: 50`. The retired referral-based proxy cost ~30-40 credits, so the default ladder is CHEAPER than what it replaced, and the expensive rung is now opt-in.
