...
3. Wrap each tool call in § error-rendering pattern 3 handling. Record a tool as access-denied ONLY on an actual access error: **HTTP 403 with `error.code` FORBIDDEN_ERROR**, consistent with the in-run tracking and persistence rules below. Key on the STATUS and CODE, never on the message text: the 403 message wording drifted between server releases (2026-06-11 "Access denied. The user might be missing the required claims for this tool." became 2026-08-06 "Access forbidden. Upgrade your account."), and the "missing the required claims" phrase moved out of `error_message` into the `actions` array. A detector gated on that substring silently misses a live claims gap. This is the OPPOSITE discipline from the country-coverage gap, where the message IS the required discriminator because a different error shares its status code. Per `partial-access-envelope-shape`. Do NOT record other 4xx responses as denials: a validation error (a 400 such as "Dates not in range", "at most 3 months", or a bad metric or param) is a schema or transient problem (handle per § error-rendering Pattern 2), and a country-coverage gap (per § Country-coverage gap detection / § Precedence) is a per-(tool, country) plan limitation, NOT a tool denial. Recording a validation error or a country gap in `tools_inaccessible` would poison the map and wrongly skip a tool the plan actually grants. An ABSENT tool (per § tool-surface presence) is likewise never recorded here; absence persists to `tools_absent` per the capability map schema.
...
**Fallback-on-denial.** A recipe MAY define a FALLBACK tool that fires when a primary tool is denied (403 claims) OR absent (per § tool-surface presence), instead of only skipping or aborting. The primary call IS the probe: attempt it, and on the 403/absence (recorded as above), run the fallback in the SAME turn; subsequent runs read the recorded denial and skip straight to the fallback. A tool that is present-in-the-list-but-403 is NOT caught by absence detection, so the try-and-catch-403 pattern is the only reliable trigger for such tools. Canonical example: sw-aeo-audit attempts `get-ai-traffic-landing-pages-agg`, then `get-ai-traffic-overview` (both are present on the live surface, and whether they return data is an ACCOUNT property: the reference connector denied them through 2026-08-06 and returned 200 from 2026-08-07, so never assume the denial branch will fire), and on a denial falls back to ONE `get-website-analysis-traffic-channels` call, reading its `Gen AI` channel row as the AI-referral proxy (absolute visits, 10 credits per month, no limit lever; divide by the summed channels for share of traffic). The `get-websites-referrals-agg` pipeline that used to serve this fallback is RETIRED, and its AI rows did not move into the surviving referral tools, so no fallback may point there. Per `referral-pipelines-divergence`.
...
- If the tool returned 403 with `error.code` FORBIDDEN_ERROR: record `(tool, domain)` as inaccessible for the rest of this turn.
...
The search-spend pattern is the canonical example: when `get-website-analysis-search-spend` succeeds on one domain and 403s on other domains, that is a domain-level restriction, not a tool-level denial.
