...
Costs above were measured on one app at the parameter shapes named in each row, and each row states the points behind it. Treat the shape as part of the claim: the family does NOT share one pricing rule, and neighbouring tools genuinely differ. Sessions and demographics are priced by the metrics list, ratings and downloads by the month, iab-categories and technographics by the returned row, and three tools are free. Before quoting a figure for a shape nobody measured, measure it: two points that sit inside the same bucket look like a model and are not one.
