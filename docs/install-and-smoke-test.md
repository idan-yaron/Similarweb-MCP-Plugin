...
- `similarweb-cowork-0.1.5.zip` (Cowork-native flagship; also accepted as `.plugin`)
- `similarweb-claude-code-0.1.5.zip` (Claude Code bundle; skills-only subset)
- `similarweb-codex-0.1.5.zip` (Codex CLI bundle; skills-only subset)
- `similarweb-cursor-0.1.5.zip` (Cursor bundle; skills-only subset)
...
What you're installing: everything the other four platforms get (7 recipes + 3 foundations + 2 operators + sw-router), plus three Cowork-only autonomous agents, three event-driven hooks, the cross-plugin connector declarations, and the artifact-pattern docs. v0.3.0 failed Cowork validation with "Plugin validation failed."; v0.1.5 fixes that by normalizing CRLF to LF at zip-time (the Cowork VM is Linux) and moving the `depends_on` ledger references out of YAML frontmatter into a `## Grounded assertions` body block (Cowork rejects non-canonical fields).
...
3. Select `similarweb-cowork-0.1.5.zip`.
...
- **"Plugin validation failed."** on upload: this was the v0.3.0 issue. v0.1.5 fixed CRLF and the depends_on frontmatter field. If you still see this on v0.1.5, log the Cowork validator output and check `python tests/structural-validation.py` locally to compare expected bundle shape against what shipped.
...
`similarweb-claude-code-0.1.5.zip`
...
1. Unzip `similarweb-claude-code-0.1.5.zip` into `~/.claude/plugins/similarweb/` (or the user-plugins directory your install uses).
...
- `similarweb-codex-0.1.5.zip` is attached to the [latest release](https://github.com/idan-yaron/Similarweb-MCP-Plugin/releases/latest); download it from there.
- `similarweb-codex-subagents-0.1.5/` (optional companion: 3 sub-agent TOMLs) is NOT attached to the official release. To get it, clone the repo and run `python3 build.py --build`; the directory and TOMLs appear under `dist/`.
...
3. In the **Add marketplace** dialog:
   ...
   - **Git ref**: `v0.1.5` (or `main` for the latest)
...
1. Download `similarweb-codex-0.1.5.zip` from the [latest release](https://github.com/idan-yaron/Similarweb-MCP-Plugin/releases/latest).
2. From the directory containing the downloaded zip, run:

```bash
unzip similarweb-codex-0.1.5.zip -d ./similarweb-codex
...
cp dist/similarweb-codex-subagents-0.1.5/*.toml ~/.codex/agents/
```
...
2. Point at the local path of `similarweb-cursor-0.1.5.zip` (or a directory containing the unzipped `.cursor-plugin/` tree).
...
- Settings > Plugins should list `similarweb` with version 0.1.5.
...
`similarweb-claude-ai-0.1.5/` (a directory, NOT a single zip)
...
Or one of the new v0.1.5 recipes:
