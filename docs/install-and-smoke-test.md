...
- `similarweb-cowork-0.1.24.zip` (Cowork-native flagship; also accepted as `.plugin`)
- `similarweb-claude-code-0.1.24.zip` (Claude Code bundle; skills + commands subset)
- `similarweb-codex-0.1.24.zip` (Codex CLI bundle; skills-only subset)
- `similarweb-cursor-0.1.24.zip` (Cursor bundle; skills + commands subset)
...
3. Select `similarweb-cowork-0.1.24.zip`.
...
`similarweb-claude-code-0.1.24.zip`
...
What you're installing: 7 user-invocable recipes (competitive-teardown, audience-overlap, aeo-audit, channel-mix, market-size, page-mix, keyword-opportunity), 1 router, 2 operator skills (sw-setup + sw-config), the 3 sub-foundation skills (sw-foundation-core / data / render), and 8 slash commands. The foundations load via each recipe's `Inherits:` block (and via sw-router when it dispatches); trivial single-domain single-metric lookups intentionally skip them for latency.
...
- `similarweb-codex-0.1.24.zip` is attached to the [latest release](https://github.com/idan-yaron/Similarweb-MCP-Plugin/releases/latest); download it from there.
- `similarweb-codex-subagents-0.1.24/` (optional companion: 3 sub-agent TOMLs) is NOT attached to the official release. To get it, clone the repo and run `python3 build.py --build`; the directory and TOMLs appear under `dist/`.
...
1. Download `similarweb-codex-0.1.24.zip` from the [latest release](https://github.com/idan-yaron/Similarweb-MCP-Plugin/releases/latest).
...
2. Point at the local path of `similarweb-cursor-0.1.24.zip` (or a directory containing the unzipped `.cursor-plugin/` tree).
...
- Settings > Plugins should list `similarweb` with version 0.1.24.
...
`similarweb-claude-ai-0.1.24/` (a directory, NOT a single zip)
