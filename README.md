...
2. Download similarweb-cowork-0.1.4.zip from the Releases page:
...
Open Cowork, click **Customize > Plugins > Upload plugin**, select `similarweb-cowork-0.1.4.zip`. The validator should accept the bundle on first try.
...
Or drop `similarweb-claude-code-0.1.4.zip` into your Claude Code plugins directory and reload. After install: `/sw-config --show` confirms the bundle is live. Skills-only subset (no sub-agents, hooks, or HTML artifacts). Requires the Similarweb MCP server registered in your Claude Code MCP config (see Prerequisite).
...
Install via `/add-plugin` from a local path pointing at `similarweb-cursor-0.1.4.zip`, or via cursor.com/marketplace if published. After install, `/sw-config` should be available in the command palette and chat. Skills-only subset. Requires the Similarweb MCP server in Cursor's MCP settings (see Prerequisite).
...
Claude.ai installs skills one at a time: **Settings > Features > Skills > upload** each of the 13 per-skill zips from `similarweb-claude-ai-0.1.4/` individually.
