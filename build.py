...
def _write_codex_readme(codex_root, plugin_name, version):
    """Emit codex/README.md explaining that this directory is the canonical Codex
    marketplace tree, auto-generated by build.py from the sources at the repo root."""
    body = (
        f"# codex/ (canonical Codex marketplace tree)\n\n"
        f"This directory is the source-of-truth Codex marketplace bundle for "
        f"`{plugin_name}` v{version}, committed to git so the Codex Desktop "
        f"**Add marketplace** dialog can fetch it directly from this repository. "
        f"Layout follows the Codex plugins spec at "
        f"https://developers.openai.com/codex/plugins/build:\n\n"
        f"```\n"
        f"codex/\n"
        f"  .agents/plugins/marketplace.json     # Codex marketplace manifest\n"
        f"  plugins/{plugin_name}/               # the plugin payload\n"
        ...
        f"Files in this directory are auto-generated by `build.py emit_codex()` "
        f"from the canonical sources at the repo root. Run `python3 build.py --build` "
        f"from the repo root to regenerate. The same step produces "
        f"`dist/similarweb-codex-<version>.zip` for the Releases page.\n\n"
        f"## How end users install via the Codex Desktop UI\n\n"
        f"1. Open Codex Desktop, click **Plugins** in the left sidebar.\n"
        f"2. Click the marketplace dropdown next to the search bar (defaults to "
        f"\"Built by OpenAI\") and choose **+ Add more**.\n"
        f"3. In the **Add marketplace** dialog:\n"
        f"   - **Source**: `idan-yaron/Similarweb-MCP-Plugin`\n"
        f"   - **Git ref**: `v{version}` (or `main` for the latest)\n"
        f"   - **Sparse paths**: `codex`\n"
        f"4. Click **Add marketplace**. The marketplace registers and the plugin "
        f"appears under the dropdown.\n"
        f"5. Open a new chat and type `compare nike.com and adidas.com on similarweb`.\n\n"
        f"See the [main README](../README.md) for the full install matrix across Cowork, "
        f"Claude Code, Codex, and Cursor.\n"
    )
    (codex_root / "README.md").write_text(body, encoding="utf-8")
