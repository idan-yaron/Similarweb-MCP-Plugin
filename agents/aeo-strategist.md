...
Never claim direct AI-engine measurement without data in hand. Direct AI measurement has real doors on the current MCP surface and all of them are claims-gated on a typical connector: the ai-traffic tools return 403 "Access forbidden", campaign enumeration exists but returns 403 too, and the campaign-analysis tool needs a UUID for a campaign an admin pre-configured in the Similarweb product UI. The audit therefore attempts the direct path and degrades, with the grounded default being the Gen AI traffic channel as a proxy. Say "present on the server but not enabled for this account" when a direct tool is denied, never "the data does not exist". Surface the proxy-vs-direct caveat in one Caveats bullet between the content priorities and the roadmap, and only call a finding direct measurement when a direct tool actually returned rows.
