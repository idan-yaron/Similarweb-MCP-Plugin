...
2. Run sw-aeo-audit by invoking the Skill tool with the recipe's skill name exactly as your platform lists it (sw-aeo-audit or its plugin-qualified form; never with a leading slash and never as a slash command), passing the target plus the keyword list. The recipe returns the proxy AEO score, per-keyword SERP landscape (brand traffic share, top non-brand competitor per keyword), AI-citation-worthy pages (URLs scored 0 to 5 by answer-box-adjacent serp_features), and a defend-keywords plus optimize-pages list. It also returns an AI-referral signal: either direct AI-traffic rows when the account has the claim, or the Gen AI traffic channel as a proxy (absolute visits plus share of all channels). Capture the top three keywords by brand traffic share, the bottom three (or brand-absent keywords), the strongest non-brand SERP rival across the keyword set, the top five AEO-worthy pages, and the Gen AI share with which rung produced it.
...
Never claim direct AI-engine measurement without data in hand. Direct AI measurement has real doors on the current MCP surface, and whether they open is an ACCOUNT property that varies, not a fact about the server. Do not assume they are gated: the reference connector returned 403 on them through 2026-08-06 and 200 from 2026-08-07. Entitlement state is stated once, in sw-foundation-core references/websites-catalog.md under the AI-referral intent; read it there rather than restating it. The campaign-analysis tool separately needs a UUID for a campaign an admin pre-configured in the Similarweb product UI, which is a setup precondition rather than an entitlement. The audit therefore attempts the direct path and degrades on an actual denial, with the fallback being the Gen AI traffic channel as a proxy. Say "present on the server but not enabled for this account" when a direct tool is denied, never "the data does not exist". Surface the proxy-vs-direct caveat in one Caveats bullet between the content priorities and the roadmap, and only call a finding direct measurement when a direct tool actually returned rows.
