...
2. Run sw-competitive-teardown by invoking the Skill tool with the target plus every named competitor as --vs arguments. The recipe will loop per domain on rank and traffic-and-engagement, run get-websites-traffic-channels per domain, and trigger a single batched get-websites-audience-overlap-agg call on the top four by rank (the live tool caps at five domains per batch). Capture from the response: every domain's monthly visits, every domain's channel mix (top three channels by share), every domain's rank in the user country, and the audience-overlap table for the top four.

3. Identify the strongest rival. Score each competitor: 60% weight on monthly visits ratio to target, 40% weight on audience-overlap share with target from the batched call. The competitor with the highest combined score is the strongest rival. If two competitors are within five points of each other, tag both and treat the higher-visits one as primary.
